...
public class DoTweenScreenShake : MonoBehaviour
{
    ...
    public RectTransform UIRig;            // full-screen RectTransform inside the Canvas
    ...
    public float WorldRotationZDegrees = 8f;
    public Vector2 UIPositionStrengthInPixels = new Vector2(45f, 45f);
    public float UIRotationZDegrees = 8f;              

    Tween wPos, wRot, uPos, uRot;

    void KillAll()
    {
        if (wPos != null && wPos.IsActive()) 
            wPos.Kill(true);
        if (wRot != null && wRot.IsActive()) 
            wRot.Kill(true);
        if (uPos != null && uPos.IsActive()) 
            uPos.Kill(true);
        if (uRot != null && uRot.IsActive()) 
            uRot.Kill(true);
    }
    ...
    public void Shake(float duration, float magnitude = 1f)
    {
        ...
        if (CameraToShake != null)
        {
            ...
            float wr = WorldRotationZDegrees * magnitude;
        ...
        if (UIRig != null)
        {
            Vector2 up = UIPositionStrengthInPixels * magnitude;
            float ur = UIRotationZDegrees * magnitude;
